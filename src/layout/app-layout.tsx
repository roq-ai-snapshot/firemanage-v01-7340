...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<FireDepartmentInterface[]>(
    ...
    () => getFireDepartments({ filter: { tenant_id: session?.user?.tenantId } }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
...

    {
      ...
      path: isTenantUser ? `/fire-departments/${data?.[0]?.id}` : '/fire-departments',
    ...
    {
      name: 'Incident Resource',
      path: '/incident-resources',
      entity: 'incident_resource',
      service: AccessServiceEnum.PROJECT,
      icon: FiBox,
    },
    { name: 'Resource', path: '/resources', entity: 'resource', service: AccessServiceEnum.PROJECT, icon: FiAnchor },
    {
