...
import { getIncidentResources, deleteIncidentResourceById } from 'apiSdk/incident-resources';
...
function IncidentResourceListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('incident_resource', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/incident-resources/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('incident_resource', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Link href={`/incident-resources/create`}>
        ...
        {isLoading ? (
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  {hasAccess('incident', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>incident</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    {hasAccess('incident', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/incidents/view/${record.incident?.id}`}>
                    ...
                    <Td>
                      {hasAccess('incident_resource', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <Td>
                          <NextLink href={`/incident-resources/edit/${record.id}`} passHref legacyBehavior>
                            <Button
                          ...
                          {hasAccess('incident_resource', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                            <IconButton
                              onClick={(e) => {
                                e.stopPropagation();
